...
class SocialFeedAdministrationController extends BaseController {
    ...
    public function saveTokenAction(Tokens $token) {
        ...
        if ($token->getUid()) {
            ...
            $title = $this->translateLabel('pxasocialfeed_module.labels.edit');
            $message = $this->translateLabel('pxasocialfeed_module.labels.changesSaved');
        } else {
            $this->tokenRepository->add($token);
            $title = $this->translateLabel('pxasocialfeed_module.labels.create');
            $message = $this->translateLabel('pxasocialfeed_module.labels.successCreated');
        }
    ...
    public function deleteTokenAction(Tokens $token) {
        ...
        if($configs->count() == 0) {
            ...
            $this->addFlashMessage($this->translateLabel('pxasocialfeed_module.labels.removedSuccess'), $this->translateLabel('pxasocialfeed_module.labels.removed'), FlashMessage::OK);
        } else {
            $this->addFlashMessage($this->translateLabel('pxasocialfeed_module.labels.cantRemoveTokenConfigExist'), $this->translateLabel('pxasocialfeed_module.labels.error'), FlashMessage::ERROR);
        }
            ...
            try {
                ...
                if ($response->getStatus() === 200) {
                    ...
                    if (isset($data['access_token'])) {
                        ...
                        $this->addFlashMessage($this->translateLabel('pxasocialfeed_module.labels.access_tokenUpdated'), $this->translateLabel('pxasocialfeed_module.labels.success'), FlashMessage::OK);
                    } else {
                        $this->addFlashMessage($this->translateLabel('pxasocialfeed_module.labels.errorGettingsToken'), $this->translateLabel('pxasocialfeed_module.labels.error'), FlashMessage::ERROR);
                    }
                } else {
                    $this->addFlashMessage($this->translateLabel('pxasocialfeed_module.labels.errorCommunication'), $this->translateLabel('pxasocialfeed_module.labels.error'), FlashMessage::ERROR);
                }
            } catch (\Exception $e) {
        ...
        if($this->view->getModuleTemplate() !== NULL) {
            ...
            foreach ($actions as $action) {
                $item = $menu->makeMenuItem()
                    ->setTitle($this->translateLabel($action['label']))
